pipeline {
  ...
  environment {
    BOT_NAME = 'report_generator_bot'
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-auditor_report_generator"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/report_generator_bot/bin/report_generator_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorReportGenerator.Release.create()'"
          $RELEASE_BIN eval 'BotArmyAuditorReportGenerator.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyAuditorReportGenerator.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyAuditorReportGenerator.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_auditor_report_generator/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_auditor_report_generator/releases/start_erl.data)
        fi
